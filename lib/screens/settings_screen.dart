...
class SettingsScreen extends ConsumerWidget {
      ...
      children: [
        ...
        _SettingsSection(
          ...
          child: _UnitToggle(
            ...
            onChanged: (unit) => _updateUnit(ref, unit),
          ),
        ),
        const SizedBox(height: 16),
        ...
        _SettingsSection(
          ...
          child: _NotificationToggles(
            ...
            onChanged: (n) => _updateNotifications(ref, n),
          ),
        ),
        const SizedBox(height: 16),
        ...
        _SettingsSection(
          ...
          child: _ScheduleEditor(
            ...
            onChanged: (s) => _updateSchedule(ref, s),
          ),
        ),
        const SizedBox(height: 16),
        ...
        _SettingsSection(
          ...
          child: _PrivacyToggles(
            ...
            onChanged: (p) => _updatePrivacy(ref, p),
          ),
        ),
      ],
    );
  ...
  Future<void> _updateTheme(WidgetRef ref, ThemeModePreference mode) async {
  ...
  Future<void> _updateUnit(WidgetRef ref, UnitSystem unit) async {
    final user = ref.read(currentUserProvider);
    if (user == null) return;
    await ref.read(preferencesServiceProvider).update(user.uid, {
  ...
  Future<void> _updateSchedule(WidgetRef ref, SchedulePreferences s) async {
    final user = ref.read(currentUserProvider);
    if (user == null) return;
    await ref.read(preferencesServiceProvider).update(user.uid, {
      'schedule': s.toMap(),
  ...
  Future<void> _updatePrivacy(WidgetRef ref, PrivacyPreferences p) async {
    final user = ref.read(currentUserProvider);
    if (user == null) return;
    await ref.read(preferencesServiceProvider).update(user.uid, {
      'privacy': p.toMap(),
