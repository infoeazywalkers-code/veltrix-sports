...
class _AthleteOnboardingScreenState extends State<AthleteOnboardingScreen> {
  ...
  Future<void> _save({required bool complete}) async {
    ...
    try {
      await OnboardingService().saveProgress(
        ...
        goalRace: _raceController.text.trim().isEmpty ? null : _raceController.text.trim(),
        complete: complete,
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(complete ? 'Profile ready. Your plan can now be personalized.' : 'Progress saved.')),
      );
      if (complete) Navigator.push(context, MaterialPageRoute(builder: (_) => trainingPlansScreen()));
    } catch (error) {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(
      ...
      children: [
        const Text('Build your athlete profile', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: navy)),
        const SizedBox(height: 8),
        const Text('Save as you go. These answers shape your training recommendations and can be updated later.'),
        const SizedBox(height: 24),
        DropdownButtonFormField<String>(
          initialValue: _sport,
          decoration: const InputDecoration(labelText: 'Primary sport'),
          items: ['Running', 'Cycling', 'Triathlon', 'Strength'].map((value) => DropdownMenuItem(value: value, child: Text(value))).toList(),
        ...
        DropdownButtonFormField<String>(
          ...
          items: ['Beginner', 'Intermediate', 'Advanced'].map((value) => DropdownMenuItem(value: value, child: Text(value))).toList(),
          onChanged: (value) => setState(() => _experience = value ?? _experience),
        ),
        const SizedBox(height: 14),
        DropdownButtonFormField<String>(
          initialValue: _goal,
          decoration: const InputDecoration(labelText: 'Main goal'),
          items: ['Build consistency', 'Race faster', 'Return from injury', 'Improve strength'].map((value) => DropdownMenuItem(value: value, child: Text(value))).toList(),
          onChanged: (value) => setState(() => _goal = value ?? _goal),
        ),
        const SizedBox(height: 14),
        TextField(controller: _raceController, decoration: const InputDecoration(labelText: 'Goal race or milestone', hintText: 'Optional')), 
        const SizedBox(height: 18),
        Text('Weekly training availability: ${_weeklyHours.round()} hours', style: const TextStyle(fontWeight: FontWeight.w800)),
        Slider(value: _weeklyHours, min: 1, max: 20, divisions: 19, label: '${_weeklyHours.round()}h', onChanged: (value) => setState(() => _weeklyHours = value)),
        const SizedBox(height: 18),
        OutlinedButton.icon(onPressed: _saving ? null : () => _save(complete: false), icon: const Icon(Icons.save_outlined), label: const Text('Save progress')),
        const SizedBox(height: 10),
        FilledButton.icon(onPressed: _saving ? null : () => _save(complete: true), icon: const Icon(Icons.check_circle_outline), label: Text(_saving ? 'Saving...' : 'Complete onboarding')),
      ],
    ),
  );
}
