...
class _EditProfileDialogState extends State<EditProfileDialog> {
  ...
  void initState() {
    ...
    _nameController = TextEditingController(
      text: widget.currentProfile?.displayName ?? FirebaseAuth.instance.currentUser?.displayName ?? '',
    );
    _selectedSports = List<String>.from(widget.currentProfile?.sports ?? ['Running', 'Cycling']);
  }
  ...
  Future<void> _save() async {
    ...
    if (_selectedSports.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select at least one primary sport.')),
      );
      return;
    }
    ...
    try {
      ...
      await UserService().update(user.uid, updatedProfile.toMap());

      if (mounted) {
        Navigator.pop(context, true);
        showFeatureMessage(context, 'Profile updated successfully.');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Primary Sports', style: TextStyle(fontWeight: FontWeight.w800, color: navy)),
